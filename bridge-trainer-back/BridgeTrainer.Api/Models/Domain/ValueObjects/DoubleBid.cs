...
{
    private sealed record DoubleBid : Bid
    {
        public override BiddingState ApplyAndValidate(BiddingState state)
        {
            var errorFound = false;
            if (state.DoubleStatus != DoubleStatus.None)
            {
                errorFound = true;
            }
            ...
            return new BiddingState(state.MaxBid, 0, state.BiddingHasEnded, DoubleStatus.X, errorFound);
        }
    }
}